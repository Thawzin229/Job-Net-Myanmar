...
<div class="container">
	...
	<div class="sixteen columns">
				...
				<div class="">
					...
					<div class="form boxed url-box">
						...
						<div class="d-flex mb-4">
							<div class="mx-2 border ps-4 pe-1 py-2 bg-success text-white" v-for="(link,index) in socials" :key="index">
						...
						<input v-model="resume.social.name" class="search-field" type="text" placeholder="Name" />
						<input v-model="resume.social.link" class="search-field" type="text" placeholder="http://" />
					...
					<a   @click="addsocial(resume.social.name,resume.social.link)" class="button gray add-url add-box"><i class="fa fa-plus-circle"></i> Add URL</a>
			...
			<div class="form with-line">
				...
				<div class="form-inside">
					...
					<div class="form boxed education-box">
						<a href="#" class="close-form remove-box button"><i class="fa fa-close"></i></a>
						<div class="d-flex mb-4">
							<div class="mx-2 border ps-4 pe-1 py-2 bg-success text-white" v-for="(edu,index) in education" :key="index">
								<div class="d-flex align-items-center">
								<div>
									{{ edu.school_name }}
								</div>
						...
						<input  v-model="resume.education.school_name" class="search-field" type="text" placeholder="School Name" value=""/>
						<input  v-model="resume.education.qualifications" class="search-field" type="text" placeholder="Qualification(s)" value=""/>
						<input  v-model="resume.education.start_and_end_date" class="search-field" type="text" placeholder="Start / end date" value=""/>
						<textarea  v-model="resume.education.note" name="desc" id="desc" cols="30" rows="10" placeholder="Notes (optional)"></textarea>
					...
					<a  @click="addEdu(resume.education.school_name,resume.education.qualifications,resume.education.start_and_end_date,resume.education.note)" class="button gray add-education add-box"><i class="fa fa-plus-circle"></i> Add Education</a>
				</div>
			</div>
			...
			<div class="form with-line">
				...
				<div class="form-inside">
					...
					<div class="form boxed experience-box">
						...
						<div class="d-flex mb-4">
							<div class="mx-2 border ps-4 pe-1 py-2 bg-success text-white" v-for="(exp,index) in experience" :key="index">
								<div class="d-flex align-items-center">
								<div>
									{{ exp.employer }}
								</div>
						...
						<input  v-model="resume.experience.employer" class="search-field" type="text" placeholder="Employer" value=""/>
						<input  v-model="resume.experience.job_title" class="search-field" type="text" placeholder="Job Title" value=""/>
						<input  v-model="resume.experience.start_and_end_date" class="search-field" type="text" placeholder="Start / end date" value=""/>
						<textarea v-model="resume.experience.note" name="desc1" id="desc1" cols="30" rows="10" placeholder="Notes (optional)"></textarea>
					...
					<a  perserve-scroll @click="addExp(resume.experience.employer,resume.experience.job_title,resume.experience.start_and_end_date,resume.experience.note)" class="button gray add-experience add-box"><i class="fa fa-plus-circle"></i> Add Experience</a>
				</div>
			</div>
      ...
      <div v-if="errors.status" v-text="errors.status" class="text-white bg-success p-2 text-center"></div>
			<a  class="button big margin-top-5" @click="add()">Preview <i class="fa fa-arrow-circle-right"></i></a>
...
methods:{
	addsocial(name,link)
	{
		if(name !== "" && link !== "")
		{
		this.socials.push({'name':name,'link':link});
		this.addedurl = true;
		setTimeout(() => {
			this.addedurl = false;
		}, 1000);
	...
	addEdu(school_name,qualifications,start_and_end_date,note)
	{
		...
		this.education.push({'school_name':school_name,'qualifications':qualifications,'start_and_end_date':start_and_end_date,'note':note});
		this.addededu = true;
		setTimeout(() => {
			this.addededu = false;
		}, 1000);
	...
	addExp(employer,job_title,start_and_end_date,note)
	{
		...
		this.experience.push({'employer':employer,'job_title':job_title,'start_and_end_date':start_and_end_date,'note':note});
		this.addedexp = true;
		setTimeout(() => {
			this.addedexp = false;
		}, 1000);
	...
	canclesocial(index)
	{
		this.socials = this.socials.slice(0, index).concat(this.socials.slice(index + 1));
	},
	cancleeducation(index)
	{
		this.education = this.education.slice(0, index).concat(this.education.slice(index + 1));
	},
	cancleexperience(index)
	{
		this.experience = this.experience.slice(0, index).concat(this.experience.slice(index + 1));
	},
	uploadImage(e)
	{
	...
	add()
	{
		this.resume.social_data = this.socials;
		this.resume.education_data = this.education;
		this.resume.experience_data = this.experience;
		this.$inertia.post("/user/resumes",this.resume,{onFinish:()=>{
      setTimeout(() => {
